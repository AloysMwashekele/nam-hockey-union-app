...
import Button from '../components/Button';
import Colors from '../constants/colors';
import { getEventRegistrations, getTeams, getEvents } from '../utils/storage';
...
const EventDetailsScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    loadRegisteredTeams();
    const unsubscribe = navigation.addListener('focus', loadRegisteredTeams);
    return unsubscribe;
  }, [eventId, navigation]);
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.scrollContainer}>
      <View style={styles.container}>
        ...
        <Text style={styles.eventInfo}>Date: {event.date}{event.endDate ? ` to ${event.endDate}` : ''}</Text>
        <Text style={styles.eventInfo}>Registration Deadline: {event.registrationDeadline}</Text>
        <Text style={styles.eventInfo}>Location: {event.location}</Text>
        <Text style={styles.eventInfo}>Fee: {event.registrationFee}</Text>
...
const styles = StyleSheet.create({
  ...
  eventTitle: { fontSize: 26, fontWeight: 'bold', marginBottom: 4, color: Colors.text },
  eventCategory: { fontSize: 18, fontStyle: 'italic', marginBottom: 12, color: Colors.primary },
  eventInfo: { fontSize: 16, marginBottom: 6, color: Colors.text },
  eventDescription: { fontSize: 14, marginVertical: 12, color: Colors.textLight },
  registrationContainer: { marginVertical: 20 },
  registrationClosed: { textAlign: 'center', color: Colors.error },
  sectionTitle: { fontSize: 20, fontWeight: 'bold', marginTop: 24, marginBottom: 12, color: Colors.text },
  teamsList: { marginBottom: 32 },
  teamCard: { marginVertical: 8 },
  teamInfo: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  teamName: { fontSize: 16, fontWeight: 'bold', color: Colors.text },
  categoryBadge: { backgroundColor: Colors.primary, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 4 },
  categoryText: { color: Colors.background, fontSize: 12, fontWeight: 'bold' },
  registrationDate: { fontSize: 14, color: Colors.gray },
});
